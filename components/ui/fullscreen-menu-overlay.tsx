      ...
      {isOpen && (
        <>
          ...
          <motion.div
            ...
            className="fixed top-0 left-0 w-full h-dvh max-h-dvh overflow-hidden z-[100] menu-bg-warm"
            onClick={onClose}
          >
            <GrainOverlay />
          </motion.div>
          ...
          <motion.div
            ...
            className="fixed top-0 left-0 w-full h-dvh max-h-dvh z-[101] flex flex-col overflow-hidden"
          >
            {/* Top bar: Language toggle (кнопка закрыть теперь в Navigation) */}
            <div className="flex-shrink-0 flex items-center justify-between px-6 md:px-8 lg:px-12 pt-2 md:pt-3 lg:pt-4 pb-0.5">
              <LanguageToggle />
              <div className="w-10 h-10 md:w-12 md:h-12 lg:w-14 lg:h-14" />
            </div>

            {/* Menu Rows — равномерно по высоте, без прокрутки, минимальные зазоры до пунктира */}
            <div className="flex-1 min-h-0 flex flex-col overflow-hidden">
              {items.map((item, index) => (
                ...
                >
                  ...
                  />
                  {/* Пунктирная линия сразу под пунктом меню */}
                  {index < items.length - 1 && (
                    <div
                      className="flex-shrink-0 w-full h-px opacity-30 mt-0.5"
                      style={{
                        backgroundImage:
            ...
            {/* Footer: слева соцсети, справа юридические ссылки */}
            <div className="flex-shrink-0 flex items-end justify-between px-6 md:px-8 lg:px-12 pb-3 md:pb-4 pt-1">
