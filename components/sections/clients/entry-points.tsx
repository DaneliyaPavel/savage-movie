...
export function EntryPoints() {
  return (
    <section className="client-entry-points border-t border-white/10 px-5 py-20 sm:px-8 md:px-10 md:py-28 lg:px-16">
      ...
      {/*
        Колонки идут лесенкой, а не в ряд: три одинаковых блока по линейке
        читаются как тарифная сетка, а этот блок — про то, что тарифа нет.
        Сдвиг задаётся индексом, на узком экране обнуляется.
      */}
      <div className="mt-12 grid gap-10 md:mt-16 md:grid-cols-3 md:gap-10">
        {ENTRY_POINTS.map((point, index) => (
          <div
            key={point.title}
            className="border-t border-white/20 pt-5"
            style={{ marginTop: `calc(${index} * var(--entry-step, 0px))` }}
          >
