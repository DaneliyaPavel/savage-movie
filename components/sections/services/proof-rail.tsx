      ...
      {items.map(work => (
        <li key={work.slug}>
          <Link
            ...
            /*
              Без предзагрузки сегмента. Раздел показывает тридцать три ссылки
              на страницы проектов, и Next тянул их заранее вместе с медиа,
              объявленным на той стороне приоритетным: один только анимированный
              превью «Охта Парка» — полтора мегабайта на первом экране раздела
              ради страницы, которую человек ещё не открыл. Переход по клику от
              этого не становится перезагрузкой: это по-прежнему клиентская
              навигация, просто сегмент запрашивается в момент нажатия.
            */
            prefetch={false}
