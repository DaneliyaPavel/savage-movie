...
export function FilmstripCarousel({ projects, onProjectSelect, selectedId }: FilmstripCarouselProps) {
  ...
  return (
    <motion.div
      ...
      className="absolute bottom-0 left-0 right-0 z-30 bg-gradient-to-t from-background/80 via-background/40 to-transparent pt-20 pb-6"
    >
        ...
        <div className="flex gap-3 md:gap-4">
          {[...projects, ...projects].map((project, index) => (
            <FilmstripItem
              ...
              noteText={HOVER_NOTES[index % HOVER_NOTES.length]}
            />
          ))}
        </div>
      </div>
    ...
    <HoverNote note={noteText}>
      ...
      >
        ...
        <div className="absolute inset-x-0 bottom-0 p-2 bg-gradient-to-t from-background/90 to-transparent">
          <p className="text-[10px] md:text-xs font-medium truncate text-foreground/90">{project.title}</p>
        </div>
      </motion.button>
    </HoverNote>
  )
