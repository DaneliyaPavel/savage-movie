...
usage() {
  echo "Usage: $0 [--restore <backup_dir>] [--no-build] [--restart-after-restore]"
}
...
if [ ! -f "$ENV_FILE" ]; then
  if [ -f "$ROOT_DIR/.env.example" ]; then
    ...
    echo "Создан $ENV_FILE из .env.example. Проверьте значения для production."
  else
...
CERT_PATH="$ROOT_DIR/infra/letsencrypt/live/$APP_DOMAIN/fullchain.pem"
if [ -f "$CERT_PATH" ]; then
...
if [ -n "$RESTORE_DIR" ]; then
  ...
  "$ROOT_DIR/scripts/restore.sh" --prod "$RESTORE_DIR"
