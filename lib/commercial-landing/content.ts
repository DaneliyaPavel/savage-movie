...
/**
 * Диапазоны бюджета — один набор на весь сайт.
 *
 * Они же используются брифом направлений на /services: человек, пришедший за
 * fashion, и человек, пришедший за рекламным роликом, должны отвечать на один
 * и тот же вопрос одинаковыми вариантами, иначе воронки невозможно сравнить.
 *
 * Границы согласованы с блоком стоимости выше и со статьёй о бюджете:
 * основной коммерческий сегмент — 350–700 тыс. ₽, расширенные кампании —
 * от 700 тыс. ₽. Вариант «пока не определён» обязателен: без него человек
 * с несформированным бюджетом просто закрывает форму.
 */
export const BUDGET_OPTIONS: readonly OptionItem[] = [
  { value: 'under-350', label: 'До 350 тыс.' },
  { value: '350-500', label: '350–500 тыс.' },
  { value: '500-700', label: '500–700 тыс.' },
  { value: '700-1000', label: '700 тыс. – 1 млн' },
  { value: '1000-plus', label: '1 млн+' },
  { value: 'undecided', label: 'Бюджет пока не определён' },
]

/**
 * Диапазоны, которые уходили в заявку до перехода на новую коммерческую
 * позицию. Сервер продолжает их принимать: запись лендинга в CMS обновляется
 * отдельно от деплоя (см. scripts/sync-commercial-pricing.mjs), и в окне между
 * ними форма ещё может прислать старое значение. Молча терять бюджет заявки
 * в этот момент нельзя — это самое ценное поле для квалификации.
 */
export const LEGACY_BUDGET_OPTIONS: readonly OptionItem[] = [
...
export const DEFAULT_COMMERCIAL_LANDING: CommercialLandingContent = {
  hero: {
    ...
    budgetQualifier: 'Большинство коммерческих проектов — 350–700 тыс. ₽',
    ctaPrimary: 'Получить предварительную смету',
    ctaSecondary: 'Смотреть проекты',
    ctaNote: 'Расскажите задачу — предложим формат производства, сроки и ориентир бюджета.',
    ...
    items: [
      {
        ...
        description: 'Рекламный ролик для digital-кампании, сайта, маркетплейсов и соцсетей.',
        projectType: 'ad',
        href: null,
      },
      {
    ...
    steps: [
      {
        ...
        items: ['изучение задачи', 'идея', 'сценарий', 'визуальная концепция', 'референсы'],
      },
      {
        number: '02',
        title: 'Препродакшн',
      ...
      },
      {
        ...
        items: ['режиссура', 'операторская работа', 'свет', 'звук', 'production management'],
      },
      {
        number: '04',
        title: 'Постпродакшн',
        ...
        items: ['монтаж', 'цвет', 'sound design', 'графика', 'VFX', 'AI/CGI при необходимости'],
      },
      {
        number: '05',
        title: 'Адаптации',
        ...
        items: ['16:9, 9:16, 1:1', 'сайт', 'digital', 'social', 'экраны', 'ТВ при необходимости'],
      },
    ],
  },
  ...
  pricing: {
    ...
    subtitle: 'Не продаём ролики по тарифу. Но порядок бюджета можно понять до первого созвона.',
    tiers: [
      {
        id: 'compact',
        ...
        range: 'до 350 тыс. ₽',
        summary:
          'Узкая задача, готовый продукт, одна локация. Иногда сюда же попадает гибрид с AI — когда генерация отдаёт кадр, который иначе не оправдать по смете. Это не полноценная кампания со скидкой, а сознательно сокращённый объём производства.',
        highlight: false,
        items: [
          ...
          'AI/hybrid, если он экономит смену',
        ],
        note: 'Здесь мы честно называем границы объёма до старта, а не по ходу проекта.',
      },
      {
        id: 'core',
      ...
      {
        id: 'extended',
        name: 'Расширенные кампании',
        range: 'от 700 тыс. ₽',
        summary:
          'Несколько смен, актёры, сложная логистика или графика — и кампания, а не один ролик. Смета считается индивидуально: слишком многое зависит от концепции, чтобы называть вилку заранее.',
        highlight: false,
        items: [
          'сложная концепция',
          'несколько смен',
    ...
    items: [
      {
        ...
        answer:
          'Основной коммерческий сегмент — 350–700 тыс. ₽: в него попадает большинство проектов Savage Movie. Расширенные кампании с несколькими сменами, актёрами и графикой — от 700 тыс. ₽. Компактный формат до 350 тыс. ₽ возможен, когда объём производства сознательно сокращён. Как складывается сумма — разобрали в отдельной статье о бюджете рекламного ролика.',
      },
      {
