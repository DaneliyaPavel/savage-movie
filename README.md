...
**SSR и запросы к API с Next-сервера:** при запуске в Docker Next.js выполняет server-side запросы (детальная страница курса, оплата, личный кабинет) к бэкенду. Для этого в среде, где крутится Next, должна быть задана переменная **`API_URL`** (или резервно `NEXT_PUBLIC_API_URL`), указывающая на доступный с Next-сервера адрес бэкенда (в Docker — например `http://backend:8000`). Иначе страницы курсов и оплаты могут отдавать 404 или пустые данные.
